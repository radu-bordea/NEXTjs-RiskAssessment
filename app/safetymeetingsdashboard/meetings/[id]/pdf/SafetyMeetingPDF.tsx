 ...
 *
 * Theme: Blue (matching Work Permits color scheme).
 */
  ...
  return (
    <Document>
      <Page size="A4" style={styles.page}>
        ...
        <View style={styles.section}>
          ...
          <View style={styles.grid}>
            {[
              ...
              { label: "Vessel / Installation",  value: meeting.vesselInstallation },
              { label: "Date",                   value: new Date(meeting.date).toLocaleDateString("en-GB") },
              { label: "Location / Area / Deck", value: meeting.locationAreaDeck },
              ...
              { label: "First time / non-routine task", value: meeting.firstTimeNonRoutine ? "Yes" : "No" },
              { label: "SIMOPS involved",        value: meeting.simopsInvolved ? "Yes" : "No" },
            ].map(({ label, value }) => (
          ...
          <View style={{ marginTop: 6 }}>
            <Text style={styles.fieldLabel}>Task Objective / Brief Description</Text>
        ...
        {/* ── Operational Context ──────────────────────────────────────── */}
        {(meeting.vesselStatus || meeting.weatherSeaState || meeting.workAreaStatus || meeting.dayNight || meeting.nearbyOperations) && (
          <View style={styles.section}>
            ...
            <View style={styles.grid}>
              ...
              {meeting.weatherSeaState && (
                <View style={styles.gridItem}>
                  ...
                  <Text style={styles.fieldValue}>{meeting.weatherSeaState}</Text>
                </View>
              )}
              {meeting.workAreaStatus && (
                <View style={styles.gridItem}>
                  <Text style={styles.fieldLabel}>Work area status</Text>
                  <Text style={styles.fieldValue}>{meeting.workAreaStatus}</Text>
                </View>
              )}
              {meeting.dayNight && (
                <View style={styles.gridItem}>
              ...
              {meeting.nearbyOperations && (
                <View style={styles.gridItem}>
                  ...
                  <Text style={styles.fieldValue}>{meeting.nearbyOperations}</Text>
                </View>
              )}
            </View>
          </View>
        ...
        {(meeting.masterOowDpo || meeting.deckPic || meeting.surveyLead || meeting.equipmentOperator || meeting.responsibleInterfacesOther) && (
          <View style={styles.section}>
            ...
            <View style={styles.grid}>
              ...
              {meeting.equipmentOperator && (
                <View style={styles.gridItem}>
                  ...
                  <Text style={styles.fieldValue}>{meeting.equipmentOperator}</Text>
                </View>
              )}
            </View>
        ...
        {meeting.selectedCards?.length > 0 && (
          <View style={styles.section}>
            <Text style={styles.sectionHeading}>Selected Toolbox Talk Cards</Text>
            {meeting.selectedCards.map((sc: any) => (
              <View key={sc.id} style={styles.cardRow}>
        ...
        {(meeting.teamConfirmations?.length > 0 || meeting.teamConfirmationsOther) && (
          <View style={styles.section}>
          ...
          <View style={styles.section}>
            ...
            <View style={styles.pillRow}>
              {meeting.teamMembers.map((m: any) => (
                <Text key={m.id} style={styles.pill}>{m.name}</Text>
              ))}
            </View>
          </View>
        )}
