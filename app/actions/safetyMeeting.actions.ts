 ...
 */
export async function saveSafetyMeetingDraft(
  ...
  try {
    const meeting = await prisma.safetyMeeting.create({
      data: {
        projectSurvey: values.projectSurvey?.trim(),
        contractNo: values.contractNo ?? null,
        vesselInstallation: values.vesselInstallation!,
...
export async function updateSafetyMeeting(
  ...
  const schema = submitAsCompleted
    ? safetyMeetingSchema
    : safetyMeetingDraftSchema;
  ...
  try {
    ...
    await prisma.safetyMeetingTeamMember.deleteMany({
      where: { safetyMeetingId: id },
    });
    await prisma.safetyMeetingCard.deleteMany({
      where: { safetyMeetingId: id },
    });
