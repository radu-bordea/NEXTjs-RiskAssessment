...
export default function RiskView({ risk }: RiskViewProps) {
  const sectionClass = "rounded-xl border border-zinc-200 dark:border-zinc-800 bg-zinc-50 dark:bg-zinc-900 p-5 md:p-6"
    ...
    <div className="w-full px-3 md:px-8 py-6 space-y-4 max-w-400 mx-auto">
      ...
      <div className="flex items-start justify-between flex-wrap gap-3 mb-2">
        <div>
          ...
          <h1 className="text-2xl md:text-3xl font-extrabold tracking-tight">{risk.ref}</h1>
          <p className="text-sm text-zinc-500 mt-1 max-w-xl">{risk.workActivity}</p>
        </div>
            ...
            >
              ...
              <div className="md:hidden p-4 space-y-3">
                <div className="flex items-center justify-between">
                  <span className="text-xs font-bold text-zinc-400 uppercase tracking-wide">Row {index + 1}</span>
                  {row.rf != null && (
                ...
                <div className="grid grid-cols-1 gap-3">
                  <div>
                    ...
                    <p className="text-sm text-zinc-900 dark:text-white">{row.hazard}</p>
                  </div>
                  <div>
                    <span className={labelClass}>Impact</span>
                    <p className="text-sm text-zinc-900 dark:text-white">{row.impact}</p>
                  </div>
                  {row.existingControls && (
                    <div>
                      <span className={labelClass}>Existing Controls</span>
                      <p className="text-sm text-zinc-500">{row.existingControls}</p>
                    </div>
                  )}
                </div>
                <div className="flex items-center gap-4 flex-wrap">
                  {row.sct && <div><span className={labelClass}>SCT</span><p className="text-xs text-zinc-600 dark:text-zinc-400">{row.sct}</p></div>}
                ...
                {row.additionalMeasures?.length > 0 && (
                  <div className="mt-2 space-y-2">
                    <p className="text-xs font-bold uppercase tracking-wide text-zinc-400">Additional Control Measures</p>
                    {row.additionalMeasures.map((m: any, mIndex: number) => (
                      <div key={m.id} className={`rounded-lg p-3 bg-zinc-50 dark:bg-zinc-900 border border-zinc-100 dark:border-zinc-800 ${getRFBorderClass(m.rfColor)}`}>
                        <p className="text-xs text-zinc-400 mb-1">Measure {mIndex + 1}</p>
                        <p className="text-sm text-zinc-900 dark:text-white mb-2">{m.furtherAction ?? "—"}</p>
                        <div className="flex items-center gap-3 flex-wrap">
                          <span className="text-xs text-zinc-400">C: <span className="font-medium text-zinc-700 dark:text-zinc-300">{m.c ?? "—"}</span></span>
                          <span className="text-xs text-zinc-400">F: <span className="font-medium text-zinc-700 dark:text-zinc-300">{m.f ?? "—"}</span></span>
                  ...
                  {!row.additionalMeasures?.length ? (
                    ...
                    row.additionalMeasures.map((m: any, mIndex: number) => (
                      <div key={m.id} className={`px-3 py-3 ${getRFBorderClass(m.rfColor)}`}>
                        <p className="text-xs text-zinc-400 mb-1 font-medium">Measure {mIndex + 1}</p>
                        <p className="text-sm text-zinc-900 dark:text-white whitespace-pre-wrap mb-2">
                          {m.furtherAction ?? "—"}
                        </p>
                        <div className="flex items-center gap-3 flex-wrap">
                          <span className="text-xs text-zinc-400">
                            C: <span className="font-medium text-zinc-700 dark:text-zinc-300">{m.c ?? "—"}</span>
                          </span>
                          <span className="text-xs text-zinc-400">
                            F: <span className="font-medium text-zinc-700 dark:text-zinc-300">{m.f ?? "—"}</span>
                          </span>
                          <div className="flex items-center gap-1.5">
                            <span className="text-xs text-zinc-400">RF:</span>
